# Model C Freeze Benchmark: soft_gradient

## Question

Model Cは `soft_gradient` guideで、noisy static guide direct displayに対して安定した再構成候補を出せるか。
...
- Date: 2026-05-16
- Experiment seed: 20260520
- Decoder seed: 4204
- Input guide: synthetic `soft_gradient` with deterministic Gaussian noise
- Input size: 32x32
- Output size: 32x32
- Model: Model C
...
| --- | ---: | ---: |
| MAD vs clean guide | 0.022560 | 0.021839 |
| Foreground mean | 0.757063 | 0.760137 |
| Background mean | 0.243025 | 0.240061 |
| Edge leakage | Not applicable | Not applicable |
| Foreground variance | 0.021767 | 0.022160 |
| Background variance | 0.021982 | 0.021742 |
| Decode time seconds | 0.000000 | 0.701323 |

Mask note: soft_gradientでは foreground/background は右半分/左半分の便宜的な分割。edge leakageは不適用。
...
列平均では大きな逆行や急な段差は見られず、今回のrunでは階調の硬い分断は目立たない。
