...
- `reading-list.md`: これから読む資料と調査テーマの一覧
- `papers.bib`: BibTeXで管理できる論文
- `links.md`: Web記事、仕様、実装例などのリンク
- `notes/`: 論文や資料ごとの読書メモ

文献そのものの主張、SIDFへの解釈、未確認の仮説は混ぜずに書く。特に「圧縮」「超解像」「創発性」に関係する資料は、SIDFで検証済みの結果と外部文献の知見を区別する。
